...
int main(int argc, char **argv){
    ...
    int send_size = (int)f_max * N / 44100 - (int)f_min * N / 44100;//28 (29 - 1)
    sample_t data_time[N];
    float data_recv[send_size * 2];
    ...
    while (1){
        int n = recv(s, data_recv, send_size * 2 * sizeof(float), 0);
        if (n == -1) {die("recv");}
        if (n == 0) {break;}

        sample_t * buf = unzip(data_recv, N, f_min, f_max, send_size);
        fwrite(buf, sizeof(sample_t), N, fp_play);
        free(buf);        
    }
